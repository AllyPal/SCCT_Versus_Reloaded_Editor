...
// Unused GE type accidentally left in; only Pipe is used. Could remove from the editor to free a key.
...
void GEKeybindSwap::ApplyGEKeybinds()
{
    ...
    static constexpr uintptr_t kZipline[] = {
        // Shift+Z is currently bound to Select All Surfaces and Zipline. As a result, placing a Zipline GE also selects all surfaces.
        // TODO: Unassign the shortcut for "Select All Surfaces" (POLY SELECT ALL) to resolve this conflict.
        // 0x10ec92b5 (GE dispatch fn) intentionally absent; controls "POLY SELECT ALL" via AccelTablePatch (VK_Z).
    ...
    // Pole - Unused GE type accidentally left in, patched once with the fixed KEY_POLE value.
